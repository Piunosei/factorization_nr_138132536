/*
Lepore factorization nr. 138132536  simple  is free software
Lepore factorization nr. 138132536 simple is Alberico Lepore creation
this file is free software
The Lepore factorization nr. 138132536 simple is free software; you can redistribute it and/or modify
...
mpz_set(copia_di_exp,exp);
 ...
 mpz_set(a,due);
 while((mpz_cmp(p,uno)==0  ||  mpz_cmp(p,N)==0 ) &&  mpz_cmp(a,max_a)<=0){
...
if(mpz_cmp(N,p)>=0){
   ...
   if(mpz_cmp(p,uno)!=0 &&  mpz_cmp(p,N)!=0 &&  mpz_cmp(mom3,zero)==0 ){
   break;
 }
 mpz_set(p,uno);////////////////////////////////////////
 mpz_add(b,b,uno);
